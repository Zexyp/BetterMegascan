...
def load_library(mdataarr: list[MegascanData],
                 ...
                 name_template_group_asset: str = None,
                 name_template_group_model: str = None,
                 catalogs_lookup: dict[str, str] = None):
        ...
        if split_models:
            if use_collections and ret["model_collections"]:
                for c in ret["model_collections"]:
                    add_asset(c, mdata, use_tags=use_tags, generate_previews=generate_previews, semantic_tags_categories=semantic_tags_categories)
                    c.asset_data.catalog_id = catalogs_lookup[mdata.categoryPath]
            else:
                for o in ret["objects"]:
                    add_asset(o, mdata, use_tags=use_tags, generate_previews=generate_previews, semantic_tags_categories=semantic_tags_categories)
                    o.asset_data.catalog_id = catalogs_lookup[mdata.categoryPath]
        else:
            if use_collections and ret["collection"]:
                ...
                ret["collection"].asset_data.catalog_id = catalogs_lookup[mdata.categoryPath]
            else:
                for o in ret["objects"]:
                    add_asset(o, mdata, use_tags=use_tags, generate_previews=generate_previews, semantic_tags_categories=semantic_tags_categories)
                    o.asset_data.catalog_id = catalogs_lookup[mdata.categoryPath]
    ...
    for mdata in [d for d in mdataarr if
        ...
        ret["material"].asset_data.catalog_id = catalogs_lookup[mdata.categoryPath]
...
def generate_cats_file(catalogs: dict[str, str]):
    cats_path = os.path.join(os.path.dirname(bpy.data.filepath), "blender_assets.cats.txt")
    assert os.path.isfile(cats_path) or not os.path.exists(cats_path)
    gen_header = not os.path.exists(cats_path)
    with open(cats_path, "a") as cats_file:
        cats_file.write("\n# Categories generated by BetterMegascan\n")
        if gen_header: # write header if file is new
            cats_file.write("VERSION 1")

        for path, id in catalogs.items():
            cats_file.write(f"{str(id)}:{path}:{os.path.basename(path)}\n")
